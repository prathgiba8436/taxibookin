...
<div class="card">
    ...
    <div class="card-body">
      ...
      <div class="form-group {{(($category->is_parent==1) ? 'd-none' : '')}}" id='parent_cat_div'>
          <label for="parent_id">Thuộc dịch vụ?</label>
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
              <option value="active" {{(($category->status=='active')? 'selected' : '')}}>Hiệu lực</option>
              <option value="inactive" {{(($category->status=='inactive')? 'selected' : '')}}>Hết hiệu lực</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
